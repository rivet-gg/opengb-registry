...
test("lock and unlock", async (ctx: TestContext) => {
    ...
    const doubleLockError = await assertRejects(async () => {
        await ctx.modules.leaderboard.lockBoard({ key: leaderboardId });
    }, RuntimeError);
    assertEquals(doubleLockError.code, "leaderboard_locked");
    ...
    const doubleUnlockError = await assertRejects(async () => {
        await ctx.modules.leaderboard.unlockBoard({ key: leaderboardId });
    }, RuntimeError);
    assertEquals(doubleUnlockError.code, "leaderboard_unlocked");
